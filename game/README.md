...
**The bow was animating backwards.** The value the old animation followed was `1` when the bow was at rest and fell to `0` as you drew, so the string crept *forward* while you nocked and snapped *back* when you loosed — exactly inside out. The whole thing is rewritten around a draw that climbs from 0 to 1 over 0.55 s and a release that snaps home in 0.16 s, and five parts now move together instead of one:

- the nock point of the string travels 26 cm
- the arrow rides on the nock rather than sliding independently of it
- both limb tips flex through 12° and spring back
- the bow itself comes up to the eye as the draw completes
- the stave rings after the loose, a decaying wobble rather than a hard stop

You cannot loose an arrow that is not drawn any more — firing before 85% of the draw does nothing and costs no arrow. The bow model was rebuilt too: a proper recurve with a riser, an arrow shelf, tips that curve away from the archer, and a lighter stave so it does not read as a black pole against bright ground.

**And somebody is holding it now.** The arm rig that reaches for the axe only ever worked for the axe, so the bow floated in front of a pair of hands that were not touching it. The bow arm now closes on the riser and stays there through the draw, the release and the kick afterwards.

The drawing hand took two tries. Aiming it at the string's actual nock put both hands within a few centimetres of each other in the middle of the screen and the two forearms crossed into a V that hid the whole bow; moving it to where a drawing hand really sits — twenty centimetres from the eye — turned it into a slab of forearm filling the bottom corner. It is posed rather than tracked in the end: it comes up beside the jaw as the draw fills, mostly lift and very little travel toward the camera, and drops away when there is nothing nocked.

**Every sound in the game is new.** I went looking for sound libraries first and came up short — Kenney gates its downloads behind JavaScript, Wikimedia Commons has fire alarms and spoken articles rather than game audio, and archive.org's search returned podcasts. Rather than assemble a mismatched pile at four sample rates and eight decibels apart, the whole set is **synthesised**, which means one sample rate, one loudness target, and loops with no seam.

**81 sounds**, generated by `tools/make_sfx.py` on top of a small DSP toolkit in `tools/audio_dsp.py`:

| | |
|---|---|
| Footsteps | 5 surfaces × 4–5 variants — grass, dirt, wood, stone, water |
| Movement | jump, two landings, crouch in and out, running out of breath |
| Axe and timber | swing, hit wood, hit stone, splitting, the creak, the fall |
| Bow | draw, release, the arrow in flight, three different impacts |
| Items | six pickups, placing, eating, swapping weapons |
| Interface | nine, including the day rolling over |
| Building | placing, removing, cycling, the door both ways |
| Fire | igniting, the crackle loop, meat sizzling, cooked |
| Animals | a call each for deer, boar and hare, plus hurt and death |
| Ambience | forest by day, forest by night, wind, water |

The footsteps are not one sample pitched around — each is a transient, a body and a surface texture built separately, so gravel does not sound like a pitched-down plank.

**Three music tracks**, written with actual chord progressions rather than a drone:

| | | |
|---|---|---|
| Menu | A min – F maj7 – C maj – E min7 | 62 bpm, 45 s |
| Exploring | eight bars around D minor | 84 bpm, 67 s |
| Day 8 onward | adds G dim and A♯, driving eighths and a taiko | 96 bpm, 58 s |

All three play at once and only their faders move, so when the score turns on day 8 it lands on the beat you were already on instead of restarting the bar.

**The sound manager was rewritten.** Twelve 2D voices and twenty-four 3D ones, pooled — felling a tree fires a dozen overlapping hits, and allocating a player for each one is both an allocation and a node the tree has to clean up mid-swing. When every voice is busy the oldest is stolen, so a burst never goes silent halfway through.

Footsteps ask the ground what it is: a short ray from just above your boots, reading water near the pond, timber on the cabin and anything you have built, stone on the rock faces, dirt in the clearings and grass everywhere else. Landing hard and landing soft are different sounds, chosen by how fast you were falling.

The day and night ambience beds crossfade off the clock over six seconds, so dusk arrives in the birdsong before you notice it in the light.

**Two mistakes worth recording**, both mine, both found by testing:

- The first pass of these sounds sat up to **11 dB apart**. I was normalising to a peak, and a transient-heavy sound like a boot on stone has a huge crest factor — the peak guard pulled its whole body down while a smooth sound like running water kept its level. Levels are set by RMS now with a soft-knee limiter rounding the peaks off, which is what a limiter is for.
- The music was landing a few dB under target because it was levelled *before* the stereo widener, and the delayed second channel changes the RMS.

**`build/syfon_audio_preview.mp3`** is a 2:41 montage of the whole set — the effects grouped the way you meet them in play, then a passage of each music cue. It reads the generated files rather than re-synthesising them, so what you hear is what the game loads.
